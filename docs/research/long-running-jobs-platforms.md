...
**Recommendation: don't reopen ADR 0003 on this evidence.**

**Decision (2026-09-06): ADR 0003 kept; RunPod not adopted as a planned second dispatcher.** The dispatcher targets Vast serverless only — no design work is spent shaping it for alternative platforms. For the record, the port would be cheap if ever needed (the job contract — workflow JSON in, request_id correlation, S3 out, webhook — maps to a RunPod handler nearly verbatim), and the signals that would justify reopening the platform question are: (a) Vast's missing liveness/retry biting in production (hung workers eating jobs), (b) the warm-floor bill dominating where RunPod's queue+FlashBoot would allow running floorless, or (c) Vast's young platform disappointing operationally. But none of that is planned-for; this doc is reference material, not a roadmap.
