...
- No worker registry/liveness bookkeeping in the core services: retry-on-crash is broker redelivery — SQS: an undeleted message "becomes visible again in the queue and can be retrieved by the same or a different consumer." https://docs.aws.amazon.com/AWSSimpleQueueService/latest/SQSDeveloperGuide/sqs-visibility-timeout.html
...
- The core services own dead-worker retry and stall detection: the SDK contract re-routes on connection failure, and the "no update for T seconds" detector is ours to build with phase-aware timeouts ([vastai-serverless.md](vastai-serverless.md) §1; [serverless-live-progress.md](serverless-live-progress.md) §6).
...
**Shape (a): DIY pool as primary, workers pull from us.** Drop serverless dispatch; the `ManagedPoolDispatcher` fallback becomes the product: our core services search offers, creates instances from the template, and each worker image gains an agent that long-polls our API or consumes a broker queue. The bill:
...
- What pull genuinely buys in this shape: dispatch bookkeeping shrinks (redelivery and fair distribution come from the broker), and a mid-job core-services restart is a non-event for workers mid-poll. Real, but small against the fleet-ops line above.
...
| Core-services bookkeeping | Less dispatch state; broker becomes worker-facing infrastructure | Job table + dispatcher state; broker (if any) stays internal-only |
