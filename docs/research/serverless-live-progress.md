...
## 8. Contradicting live evidence: the POC probed the stream and ruled it out for progress

The POC ran a live probe of `/generate/stream` on 2026-08-03 (PLAN.md, "Decisions taken", in `Astral100/SeedVr`; capture retained at `logs/wrapper-stream-74af0990b28347208b3447221cfe35a8.log`): the route is SSE (`text/event-stream`, `data: {json}` events carrying `request_id`/`status`/`message`/`timestamp` plus queue info) but **emitted lifecycle transitions only** — after "queued" and "processing" it stayed silent through 30 s of generation while `/result` polling already reported 20%. The POC therefore kept wrapper tracking on `/result` polling, whose `message` field carries structured percent ("Progress: 70.0% (70/100)", parsed by `WrapperMessageParser`).

This contradicts §3/§5's ≤2 s-cadence expectation for the stream, which was read from ai-dock's current source: either the POC's deployed wrapper build predates message-cadence stream emission, or `_stream_status_updates` wakes only on `status` transitions (not `message` updates). Until a live re-probe of the exact image we bake proves otherwise, **option 1 must be treated as lifecycle-only** — it would need a wrapper-side fix as well as the pyworker route registration, erasing its "smallest change" advantage.

Net effect on the ranking in §5: **option 2 (worker→backend push hooked at `_update_progress`) is the clear leading candidate** — the same funnel that feeds `/result`'s message is a live-verified carrier of percent, it needs only the wrapper fork we already plan, and it retains the crash-resilience and heartbeat advantages already noted.
